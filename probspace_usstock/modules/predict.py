...
class PredictModule:
    def __init__(
        ...
        """
        self._primary_key = config.primary_key
        self._pred_cols = config.pred_cols
        self._feature_columns = config.feature_columns.keys()
    ...
    def predict(self, model, test_df: pd.DataFrame, cv_num: int = 1) -> pd.DataFrame:
        ...
        X_test = test_df[self._feature_columns].drop(self._primary_key, axis=1).copy()

        # log
        print(f"pred {self._pred_cols} ...")
        # model_type別に処理
        if self._model_type == "lightgbm":
        ...
        # 予測値をdfに書き込む
        pred_col = f"pred_{self._pred_cols}"
