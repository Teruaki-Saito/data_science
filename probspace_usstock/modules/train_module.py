...
class TrainLGBModule(AbsTrainModule):
    ...
    def train_model(
        ...
        X_train = df_train[self._feature_columns.keys()].drop(self._primary_key, axis=1).copy()
        X_val = df_val[self._feature_columns.keys()].drop(self._primary_key, axis=1).copy()
...
def calc_weight(input_df: pd.DataFrame, lgb_weight: str) -> pd.DataFrame:
    ...
    """
    weight = 1 / input_df[lgb_weight].replace(0, 1) * 10
    return weight
