...
class AbsTrainModule(metaclass=ABCMeta):
    def __init__(
        ...
        self._weight_column = None if config.weight_column is None else config.weight_column
        self._target = target
...
class TrainLGBModule(AbsTrainModule):
    ...
    def train_model(
        ...
        Args:
            ...
            have_weight (bool, optional): _description_. Defaults to False.

        Returns:
            _type_: _description_
        """
        # yを取り出す
        y_train = train[self._target].copy()
        y_val = valid[self._target].copy()
        X_train = train[self._feature_columns.keys()].copy()
        X_val = valid[self._feature_columns.keys()].copy()
